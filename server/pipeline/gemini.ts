...
import { getVideoModel } from "../keys";
...
export async function transcribeAudio(
  ...
  console.log(`[transcribeAudio] Running Whisper (${modelSize}) on: ${audioPath}`);
  let stdout: string;
  try {
  ...
  if (!jsonStr) {
    throw new Error("Whisper returned no JSON output");
  }

  let parsed: { text?: string; words?: Array<{ word: string; start: number; end: number }>; duration?: number };
  try {
    parsed = JSON.parse(jsonStr);
  } catch (e) {
    throw new Error(`Whisper JSON parse error: ${e instanceof Error ? e.message : String(e)}`);
  }

  const durationArgs = ["-v", "error", "-show_entries", "format=duration", "-of", "csv=p=0", audioPath];
  let durationStr: string;
  try {
    ({ stdout: durationStr } = await execFileAsync("ffprobe", durationArgs));
  } catch (error) {
    throw new Error(getCommandErrorMessage(error, "ffprobe"));
  }
  const duration = parseFloat(durationStr.trim()) || parsed.duration || 0;

  const rawWords = parsed.words || [];
  const fixedWords = fixWordTimestamps(rawWords, duration);

  console.log(`[transcribeAudio] Whisper done: ${fixedWords.length} words, ${duration.toFixed(1)}s`);
...
export async function extractHighlights(
  ...
  const text = `You are analyzing a ${formatTimestamp(videoDuration)} long video/podcast.
The transcript is:
"${transcript}"
...
export async function curateVideoSegments(
  ...
  // Prepare a smaller analysis copy of the video for Gemini
  const analysisVideoPath = await prepareAnalysisVideo(sourceVideoPath, videoDuration);

  const voiceoverTimeline = formatVoiceoverTimeline(voiceoverWords);
  const numSegments = Math.max(3, Math.ceil(targetDuration / 4));
...
[{"start":"00:01:23","end":"00:01:27"},{"start":"00:03:45","end":"00:03:49"}]`;
  ...
  // Build the content with the actual video
  ...
  try {
    ...
    throw new Error(`Failed to read analysis video: ${err instanceof Error ? err.message : err}`);
  }
  ...
  // Cleanup analysis video immediately after reading into memory
  try { fs.unlinkSync(analysisVideoPath); } catch {}
  try { fs.rmdirSync(path.dirname(analysisVideoPath)); } catch {}
  ...
  throw new Error("Failed to parse Gemini segments: unreachable");
}
...
export async function findHookMoment(
  ...
  // We only analyze the first 60 seconds to find the hook so Gemini answers instantly
  const analysisVideoPath = await prepareAnalysisVideo(sourceVideoPath, videoDuration, 60);
...
Maximum timestamp: ${formatTimestamp(Math.min(videoDuration, 60))}.`;
  ...
  try {
    ...
    try { fs.unlinkSync(analysisVideoPath); } catch {}
    try { fs.rmdirSync(path.dirname(analysisVideoPath)); } catch {}
    throw new Error(`Failed to read analysis video for hook: ${err instanceof Error ? err.message : err}`);
  }
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    try {
      ...
      const isLast = attempt === maxAttempts - 1;
      console.warn(`[findHookMoment] Attempt ${attempt + 1}/${maxAttempts} failed:`, err instanceof Error ? err.message : err);
      if (isLast) throw err;
