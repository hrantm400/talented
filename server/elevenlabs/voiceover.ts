...
function planToModel(plan: string): string {
  // free и paid оба используют актуальную multilingual модель, как в прошлой версии
...
async function generateVoiceoverInner(
  ...
  const { apiKey, plan } = await getElevenLabsKey(userId);
