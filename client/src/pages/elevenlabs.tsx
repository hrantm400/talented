...
import { Mic2, Loader2, Sparkles, Star, ExternalLink, Upload } from "lucide-react";
...
export default function ElevenLabsPage() {
  ...
  const [cloneSamples, setCloneSamples] = useState<FileList | null>(null);
  const [cloning, setCloning] = useState(false);

  async function handleClone() {
    if (!cloneName.trim() || !cloneSamples?.length) return;
    setCloning(true);
    try {
      const fd = new FormData();
      fd.append("name", cloneName.trim());
      fd.append("description", "Cloned via ReelForge");
      Array.from(cloneSamples).forEach((f) => fd.append("samples", f));
      ...
      toast({ title: "Voice cloned", description: `New voice id: ${json.voice_id}` });
      setCloneName("");
      setCloneSamples(null);
    ...
    <div className="flex-1 w-full max-w-4xl mx-auto px-4 sm:px-6 py-8 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-[2fr,1fr] gap-4 items-start">
        <Card className="p-6 space-y-4">
          ...
          {/* Voice Lab — clone a voice from samples */}
          <div className="border-t pt-4 mt-4 space-y-3">
            ...
            <p className="text-xs text-muted-foreground">
              Upload 1-5 audio samples (≥ 30s total recommended) and ElevenLabs will create a new voice you can
              use in any voiceover. Requires a paid plan with voice-cloning slots.
            </p>
            <Input
              value={cloneName}
            ...
            <input
              ...
              accept="audio/*"
              multiple
              onChange={(e) => setCloneSamples(e.target.files)}
              className="text-xs"
            />
            <Button
              onClick={handleClone}
              disabled={cloning || !cloneName.trim() || !cloneSamples?.length}
              className="w-full"
            >
              {cloning ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <Upload className="w-4 h-4 mr-2" />}
              Clone Voice
            </Button>
          </div>
        </Card>
