...
export default function AutomatedShortsPage() {
  ...
  const handleBulkImport = (rows: Array<{ name: string; fullUrl: string; shortUrl: string; vertical: boolean; hook: boolean }>) => {
...
const EMPTY_ROW = (): BulkRow => ({ fullUrl: "", name: "", shortUrl: "", vertical: false, hook: false });
...
function parseSheetText(text: string): BulkRow[] {
  const lines = text.split("\n").filter((l) => l.trim());
  if (!lines.length) return [];

  let startIdx = 0;
  const firstLower = lines[0].toLowerCase();
  if (/name|full|short|url|video|vertical|hook/.test(firstLower)) {
    startIdx = 1;
  }
  ...
  return lines.slice(startIdx).map((line) => {
    ...
    return {
      ...
      name: (parts[1] ?? "").trim(),
      shortUrl: (parts[2] ?? "").trim(),
      vertical: isTruthy(parts[3] ?? ""),
      hook: isTruthy(parts[4] ?? ""),
    };
  });
}
...
function BulkPasteDialog({
  ...
  const updateRow = (i: number, field: keyof BulkRow, value: string | boolean) => {
    setRows((prev) => {
      const next = [...prev];
  ...
  const handleCellPaste = (
    ...
    // Columns in grid order: 0=fullUrl, 1=name, 2=shortUrl, 3=vertical, 4=hook
    const colKeys: Array<keyof BulkRow> = ["fullUrl", "name", "shortUrl", "vertical", "hook"];
        ...
        <DialogHeader>
          ...
          <DialogDescription>
            ...
            <code className="px-1 py-0.5 bg-muted rounded text-xs">
              Full URL → Name → Short URL → Vertical → Hook
            </code>
          </DialogDescription>
        </DialogHeader>
              ...
              <table className="w-full text-sm border-collapse">
                ...
                <tbody>
                  {rows.map((row, i) => {
                    ...
                    return (
                      <tr key={i} className={`border-t border-border group ${isFallback ? "bg-amber-50/40" : ""}`}>
